...
# Cell-type order
ct1 = ['CD14Mono','CD16Mono','cDC','pDC','CD4T','CD8T','NK','T_Prolif','B','Plasma']
mye = ['CD14Mono','CD16Mono','cDC1','cDC2','pDC']
lym = ['CD4T_Naive','CD4T_Mem','CD4T_reg','CD8T_Naive','CD8T_Mem',
       'CD8T_MAIT','CD8T_Cytotoxic','NK_Dim','NK_Bright']
B = ['B_Naive','B_Mem','B_Atypical','Plasma','B_Plasma_cycling']
ct2 = mye + lym + B + ['T_Prolif']

# Colors
ct1_color = ['#FFDAB9','#CD6600', '#8B4500', '#FF8C00','#3D83C5','#A2D8E5','#917FBA','#696969','#F1A1C2','#E7211A']
mye_color = ['#FFDAB9','#CD6600','#8B4500','#E9967A','#FF8C00']
lym_color = ['#0000EE','#00008B','#1E90FF','#00F5FF','#98F5FF',
             '#00CED1','#7AC5CD','#AB82FF','#E066FF']
B_color = ['#8B0000','#FF69B4','#CD5C5C','#FF0000','#FFB6C1']
ct2_color = mye_color + lym_color + B_color + ['#696969']

MasterORDER_dict = {'Celltype1': ct1, 'Celltype2': ct2}
colorrSS_dict = {'Celltype1': ct1_color, 'Celltype2': ct2_color}
...
def Celltype_pro_plot(covar, ct_name, order_list, group1_number, group2_number):
    ...
    col_wrap_dict = {'Celltype1': 5, 'Celltype2': 5}
...
# Run
res_ct1 = Celltype_pro_plot('Disease', 'Celltype1', ['HC','T1D'], 0, 1)
res_ct2 = Celltype_pro_plot('Disease', 'Celltype2', ['HC','T1D'], 0, 1)
